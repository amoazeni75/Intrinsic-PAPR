...
from intrinsic_papr.data.pipeline import (
    calculate_shading_from_albedo_and_rendered_image,
    read_exr_with_alpha,
    tone_map_image,
)
...
def save_image(img, alpha_channel, img_name, args, save_raw_images=False):
    # Name the output after the source frame's stem plus the format actually being
    # written, never the source frame's own extension. Mip-NeRF 360 ships .JPG, and
    # keeping that extension produced a PNG called .JPG and a "<name>.JPG.npy" that
    # the loaders cannot find.
    stem = os.path.splitext(img_name)[0]
    if alpha_channel is not None:
        img = np.dstack([img, alpha_channel])
    if save_raw_images:
        np.save("{}.npy".format(stem), img)
    else:
        img = Image.fromarray((np.clip(img, 0, 1) * 255.0).astype(np.uint8))
        img.save("{}.{}".format(stem, args.format))
        ...
        for img_index in pbar:
            ...
            if args.save_render and args.save_raw_images:
                save_image(
                    img=raw_render[0],
                    alpha_channel=alpha_channel,
                    # write the raw render beside the frame it came from. Rebuilding
                    # this as dataset_root/<split>/ breaks --dataset_type custom, whose
                    # split is "custom" and has no directory of that name.
                    img_name=render_img_paths[split][img_index],
                    args=args,
                    save_raw_images=True,
                )
