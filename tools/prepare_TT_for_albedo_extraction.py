...
from intrinsic_papr.data.pipeline import (
    load_resize_normal_image,
    make_img_bg_transparent,
)
...
parser.add_argument(
    ...
    nargs="*",
    default=["Ignatius"],
    help="Scenes whose background is painted white instead of made transparent",
)
...
white_bg_scenes = [d for d in datasets if d in _args.white_bg_scenes]
transparent_bg_scenes = [d for d in datasets if d not in _args.white_bg_scenes]
...
for dataset in transparent_bg_scenes:
...
for dataset in white_bg_scenes:
