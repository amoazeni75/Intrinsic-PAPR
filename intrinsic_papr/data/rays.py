...
from .loaders import load_blender_data, load_mip360_data, load_t2_data
...
def extract_patches(
    imgs, albedos, rays_o, rays_d, dataset_args, alpha_channel, keep_patches=None
):
    """Cut `patches.max_patches` random patches out of each image.

    `keep_patches` caps how many of them are actually written out. The random
    offset of every patch is still drawn either way, so the random stream does
    not depend on this argument - a caller that reads only the first patch gets
    exactly the same pixels, without paying to copy the ones it discards.
    None keeps them all.
    """
    ...
    stored_patches = (
        num_patches if keep_patches is None else min(keep_patches, num_patches)
    )
    rayd_patches = np.zeros(
        (N, stored_patches, patch_opt.height, patch_opt.width, 3), dtype=np.float32
    )
    rayo_patches = np.zeros((N, stored_patches, 3), dtype=np.float32)
    img_patches = np.zeros(
        (N, stored_patches, patch_opt.height, patch_opt.width, C), dtype=np.float32
    )
    if albedos is None:
        albedo_patches = None
    else:
        num_cIMLE_samples = albedos.shape[1]
        albedo_patches = np.zeros(
            (N, num_cIMLE_samples, stored_patches, patch_opt.height, patch_opt.width, C),
            dtype=np.float32,
        )
    alpha_channel_patches = np.zeros(
        (N, stored_patches, patch_opt.height, patch_opt.width, 1), dtype=np.float32
    )

    for i in range(N):
        for n_patch in range(num_patches):
            # Draw for every patch, including the ones that are not stored:
            # the number and order of these draws is part of the training
            # stream, so skipping them would change the run.
            start_height = np.random.randint(0, H - patch_opt.height)
            start_width = np.random.randint(0, W - patch_opt.width)
            if n_patch >= stored_patches:
                continue
