...
from chrislib.color_util import iuv2rgb, srgb_to_rgb
...
# this function is specific to the Hypersim dataset so I'm putting it here
...
def load_exr(path):
    return cv2.imread(path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)[:, :, ::-1]


def random_degrade(full, kern):
    # full comes in as 1 x H x W
    full = full[0, :, :].numpy()

    th, tw = full.shape

    noise = (np.random.rand(10, 10) > 0.5).astype(float)
    noise = cv2.resize(noise, (tw, th))

    freq = np.fft.fft2(noise)
    freq_kernel = np.fft.fft2(np.fft.ifftshift(kern))
    convolved = freq * freq_kernel
    out = np.real(np.fft.ifft2(convolved)).astype(np.float32)

    out -= out.min()
    out /= out.max()

    return torch.from_numpy(full * out).unsqueeze(0)


def match_albedo(img, alb, ord_shd, msk):
    # img: 3 x H x W
    # alb.shape: 3 x H x W
    # ord_shd.shape: 3 x H x W
    # msk.shape: 1 x H x W

    ord_base = ord_shd[0, :, :].numpy()  # H x W
    img = img.permute(1, 2, 0).numpy()
    alb = alb.permute(1, 2, 0).numpy()
    msk = msk.squeeze(0).numpy()  # H x W

    rand_msk = np.random.randn(*msk.shape) > 0.75
    rand_msk = msk & rand_msk

    base_alb = img / uninvert(ord_base[:, :, None]).clip(1e-5)

    # compute the brightness of the three albedos so we can
    # find two scalars that matches them up optimally.
    flat_base = get_brightness(base_alb)[np.where(rand_msk == 1)]
    flat_gt = get_brightness(alb)[np.where(rand_msk == 1)]

    # compute least squares scalar that matches the ground truth to the base prediction and full prediction to the base prediction
    scale, _, _, _ = np.linalg.lstsq(flat_gt.reshape(-1, 1), flat_base, rcond=None)

    scaled_alb = alb * scale
    return torch.from_numpy(scaled_alb).permute(2, 0, 1)
...
class GenericColorfulDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # if we aren't using a cached datapoint, so either sample one or use the passed idx
            ...
            if self.random:
                file_idx = random.randint(0, self.dataset_len - 1)
            else:
                file_idx = idx
...
class HypersimColorfulDataset(GenericColorfulDataset):
    def __init__(self, root_dir, split_file, stage, ignore_list, **kwargs):
...
class GTAColorfulDataset(GenericColorfulDataset):
    def __init__(self, root_dir, stage, **kwargs):
        self.root_dir = root_dir
        super().__init__(stage, **kwargs)

    def populate_file_list(self):
        return glob(f"{self.root_dir}/*/*_final.png")

    def get_shd_fname(self, fname):
        return fname.replace("fsvg", "fsvg_albedo").replace("final", "gry_shd")

    def get_ord_fname(self, fname):
        return fname.replace("fsvg", "fsvg_albedo").replace("final", "ord_shd")

    def load_inp_alb_msk(self, fname):
        inp = load_image(fname) ** 2.2
        alb = (
            load_image(fname.replace("fsvg", "fsvg_albedo").replace("final", "albedo"))
            ** 2.2
        )
...
class InteriorVerseColorfulDataset(GenericColorfulDataset):
    ...
    def get_shd_fname(self, fname):
        fname = f"{self.root_dir}/{fname}"
        return fname.replace("_im.exr", "_gry_shd.png")

    def get_ord_fname(self, fname):
        fname = f"{self.root_dir}/{fname}"
        return fname.replace("_im.exr", "_ord_shd.png")

    def load_inp_alb_msk(self, fname):
        fname = f"{self.root_dir}/{fname}"

        hdr_inp = cv2.cvtColor(cv2.imread(fname, -1), cv2.COLOR_BGR2RGB)
        hdr_alb = cv2.cvtColor(
            cv2.imread(fname.replace("_im", "_albedo"), -1), cv2.COLOR_BGR2RGB
        )
        msk = cv2.imread(fname.replace("_im", "_mask"), -1).astype(np.uint8)
        ...
        max_alb = alb.max()
        if max_alb == 0:
            # this means it's a bad datapoint (zero albedo) so draw a new one
            return None
        ...
        # any pixels that have a very low value across all channels
        # should be masked out, we expand these regions a bit to be safe
        msk *= (alb > MIN_VAL).any(axis=-1, keepdims=False).astype(np.uint8)
...
class EdenColorfulDataset(GenericColorfulDataset):
    def __init__(self, root_dir, stage, **kwargs):
        self.root_dir = root_dir
        self.stage = stage
        super().__init__(stage, **kwargs)
    ...
    def populate_file_list(self):
        return glob(f"{self.root_dir}/*/*/*_L.png")

    def get_shd_fname(self, fname):
        return fname.replace("_L.png", "_gry_shd.png")

    def get_ord_fname(self, fname):
        return fname.replace("_L.png", "_ord_shd.png")

    def load_inp_alb_msk(self, fname):
        inp = srgb_to_rgb(load_image(fname))
        alb = load_image(fname.replace("_L.png", "_L_Diffuse_color.png"))
...
class Structure3DColorfulDataset(GenericColorfulDataset):
    def __init__(self, root_dir, split_file, stage, **kwargs):
        self.split_file = split_file
        self.root_dir = root_dir
        super().__init__(stage, **kwargs)
    ...
    def get_shd_fname(self, fname):
        fname = f"{self.root_dir}/{fname}"
        return fname.replace("rgb_rawlight.png", "gry_shd.png")

    def get_ord_fname(self, fname):
        fname = f"{self.root_dir}/{fname}"
        return fname.replace("rgb_rawlight.png", "ord_shd.png")

    def load_inp_alb_msk(self, fname):
        fname = f"{self.root_dir}/{fname}"
        inp = load_image(fname)[:, :, :3] ** 2.2
        alb = (
            load_image(fname.replace("rgb_rawlight.png", "albedo.png"))[:, :, :3] ** 2.2
        )
...
class MatrixCityColorfulDataset(GenericColorfulDataset):
    def __init__(self, root_dir, stage, **kwargs):
        self.root_dir = root_dir
        super().__init__(stage, **kwargs)

    def populate_file_list(self):
        return glob(f"{self.root_dir}/small_city_diffuse/*/*/*/*.png")

    def get_shd_fname(self, fname):
        return fname.replace("_diffuse", "").replace(".png", "_gry_shd.png")

    def get_ord_fname(self, fname):
        return fname.replace("_diffuse", "").replace(".png", "_ord_shd.png")

    def load_inp_alb_msk(self, fname):
        inp = load_image(fname.replace("_diffuse", ""))[:, :, :3] ** 2.2
        alb = load_image(fname)[:, :, :3] ** 2.2
...
class PRIDColorfulDataset(GenericColorfulDataset):
    def __init__(self, root_dir, split_file, stage, **kwargs):
        self.root_dir = root_dir
        self.split_file = split_file
        super().__init__(stage, **kwargs)
    ...
    def get_shd_fname(self, fname):
        fname = f"{self.root_dir}/{fname}"
        return fname.replace("input-resize", "pred_shd")

    def get_ord_fname(self, fname):
        fname = f"{self.root_dir}/{fname}"
        return fname.replace("input-resize", "ord_shd")

    def load_inp_alb_msk(self, fname):
        fname = f"{self.root_dir}/{fname}"
        inp = load_image(fname) ** 2.2
        alb = load_image(fname.replace("input-resize", "diffuse-resize")) ** 2.2
        msk = load_image(fname.replace("input-resize", "mask-resize"))
...
class LumosColorfulDataset(GenericColorfulDataset):
    def __init__(self, root_dir, stage, **kwargs):
        self.root_dir = root_dir
        super().__init__(stage, **kwargs)
    ...
    def populate_file_list(self):
        ...
        file_list = glob(f"{self.root_dir}/img0/*/*/*.exr")
        return file_list

    def get_shd_fname(self, fname):
        return fname.replace("img0", "pred_shd").replace("exr", "png")

    def get_ord_fname(self, fname):
        return fname.replace("img0", "ord_shd").replace("exr", "png")

    def load_inp_alb_msk(self, fname):
        hdr_inp = cv2.cvtColor(cv2.imread(fname, -1), cv2.COLOR_BGR2RGB)
        alb = load_image(fname.replace("img0", "albedo").replace("exr", "jpg"))
        msk = load_image(fname.replace("img0", "mask").replace("exr", "jpg"))[:, :, 0]

        # convert the matte into a hard binary mask (represented as floats)
        msk = (msk == 1).astype(np.single)

        # now we preprocess the datapoint to get the tonemapped albedo and image
        # and subsequently the implied shading, all in full-percision
